...
This is the place to flag the genuine novelty, because it cuts against a stronger instinct than the Python one. If you have written Java or C++, [[nominal|nominal-vs-structural]] typing is all you have ever known: a Java class is a `Point` only if it was *declared* `class Point` or `implements Point`, and a C++ `struct Point` is a distinct type from a byte-identical `struct Coord` — the name is the identity, and two unrelated types with the same fields are never interchangeable, ever. TypeScript discards that rule wholesale. Two differently-named shapes with the same members are the *same type*, and there is no keyword that makes a type refuse a value purely because of where it came from (you fake one later, with [[branding|branded-types]]). That is the deliberate-novelty flag: structural typing has no equivalent in the nominal languages, so there is no prior mental model to port — only one to set down.
