...
There's a second axis worth setting straight if you've used C++ or Java, because it decides what `<T>` *costs* at runtime — and TypeScript's answer matches Java's, not C++'s. A C++ template is *monomorphized*: `first<int>` and `first<string>` compile to two separate, specialized functions in the binary, which is why templates are fast and why their error messages are infamous. Java went the other way with *type erasure*: `List<String>` and `List<Integer>` are the same `List` class at runtime, the parameter checked at compile time and then thrown away — which is why Java can't write `new T()` or `x instanceof List<String>`. TypeScript is erasure taken to its limit: not just the type *argument* but the entire type system vanishes (Lesson 01), so `first<number>` and `first<string>` are the *same* JavaScript function and there is no `T` to inspect at runtime at all. Every limitation that follows from erasure in Java — no `new T`, no runtime check of a generic — applies here for the same reason, only more so.

This is the practical gap between TypeScript and Python's typing. Python's `TypeVar` exists mainly so a checker ([[mypy]], pyright) can verify your annotations after the fact; you still annotate, the checker still mostly confirms. TypeScript's inference is doing the same job a checker does, but it runs as you type and fills in the type arguments for you. The algorithm is [[Hindley–Milner|hindley-milner]]–inspired rather than pure [[HM|hindley-milner]] — TypeScript layers subtyping, unions, and [[structural typing|structural-typing]] on top, which is why it occasionally needs a hint where Haskell wouldn't — but for the everyday case of "solve `T` from the argument," it just works.
