...
This is the cure for the most famous null bug in the industry. In Java, every reference type silently admits `null`, so a `String name` can be `null` and `name.length()` throws the `NullPointerException` that Tony Hoare called his "billion-dollar mistake" — the type said `String`, the value said nothing. C++'s `T*` is the same hole, where a `nullptr` dereference is undefined behavior rather than even a clean crash; modern Java reaches for `Optional<String>` and C++ for `std::optional<T>` to make the absence visible in the type, but only by opting in, exactly as Python's `Optional` does. TypeScript's `strict` flips the default the other way: nothing is nullable unless you wrote `| null` or `| undefined`, so the unchecked dereference can't compile in the first place. This inverts the Python default you carry in too. In Python, `def find(id: int) -> User` will happily `return None` at runtime — the annotation is a no-op. Whether anything complains is up to [[mypy]], and even there the check is recent: `strict_optional` is on by default in current [[mypy]], but for years it was opt-in, and the language itself still treats `None` as assignable to anything. TypeScript leaves no such gap: non-null is the default and optionality is what you opt into. A function annotated `: User` cannot return `undefined`; if it might, its return type is `User | undefined`, and the compiler — not a separate linter you may or may not run — flags every path that violates it until you say so.
