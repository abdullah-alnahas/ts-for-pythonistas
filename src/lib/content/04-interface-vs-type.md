...
Lesson 03 left a question open: [[structural typing|structural-typing]] decides whether a value *fits* a shape, but when you give a shape a name, does the name itself matter — and are there two ways to write that name that behave differently? TypeScript has exactly two naming tools, `interface` and `type`, and for the common case — naming a plain object shape — they are interchangeable: which you pick changes nothing about how your code type-checks. The interest is in the handful of places they stop being interchangeable, because each of those seams exists for a concrete reason in the compiler.

Coming from Python this looks like a simplification, and it is. Python hands you a drawer of overlapping shape tools: `TypedDict` for dict-shaped data, `dataclass` and `NamedTuple` for instances, [[Protocol|typing-protocol]] for [[structural|structural-typing]] matching, and `TypeAlias` for naming an arbitrary type. They differ partly in intent and partly in what they emit at runtime. TypeScript splits the naming job along a single axis instead, and that axis is worth stating up front because every later difference follows from it: `type` names *any* type; `interface` names only an *object or function shape*.
...
Both forms produce a type that any value with a `name: string` and `age: number` satisfies — the [[structural|structural-typing]] rule from Lesson 03, unchanged by the choice. `TypedDict` is the closest single Python analog to either: a compile-time description of a dict's keys with no runtime class behind it. The rest of the lesson walks the seams where the two TS forms diverge, one at a time.
...
:::answer
(A). Two things are wrong with it, and the deeper one is the point. The surface bug is the `=`: an `interface` never uses assignment syntax — its body is a brace block of members. The structural reason is that even with correct syntax there is nowhere to put a union *inside* an interface, because an interface is a list of members and a union is not a member. Unions, tuples, bare primitive aliases, and anything computed (`keyof`, [[mapped|mapped-types]], [[conditional types|conditional-types]]) must be `type`. This single gap drives most "which do I use" decisions: if the thing you are naming is not an object or function shape, the choice is made for you.
:::
...
The results are equivalent here, but the two routes are not the same operation, and the difference decides which one you reach for. `extends` records a *relationship*: the compiler remembers that `Dog` is `Animal` plus some members, keeps `Dog` as a named, cached entity, and checks assignments against that named shape. `&` *computes* a new merged shape eagerly and gives it your alias as a label. Three consequences follow, all practical:
