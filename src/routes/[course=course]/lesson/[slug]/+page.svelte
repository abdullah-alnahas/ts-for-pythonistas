<script lang="ts">
	...
	const doneCount = $derived(getDoneCount(courseId));
	...
	function markDone() {
		const wasDone = isDone(courseId, lesson.slug);
		toggleDone(courseId, lesson.slug);
	...
	$effect(() => {
		...
		if (slug !== lastSeenSlug) {
			...
			untrack(() => {
				justCompleted = false;
				setLastViewed(courseId, slug, 0); // record arrival; scroll updates below
			});
		}
	});
	...
	onMount(() => {
		...
		function onScroll() {
			...
			raf = requestAnimationFrame(() => setLastViewed(courseId, lesson.slug, window.scrollY));
		}
	...
	<div class="page lesson-page">
		<main class="lesson">
			...
			<div class="lesson-foot">
				...
				{#if justCompleted}
					...
					<div class="done-beat" role="status">
						...
						{#if doneCount >= totalLessons}
							<a class="btn primary done-beat-cta" href="{base}/{courseSlug}">Back to course →</a>
						{:else if data.next}
							<a class="btn primary done-beat-cta" href="{base}/{courseSlug}/lesson/{data.next.slug}">
				...
				<div class="nav-row">
					{#if data.prev}
						<a class="navlink prev" href="{base}/{courseSlug}/lesson/{data.prev.slug}">
							<span class="navlink-dir">← Previous</span>
							<span class="navlink-title">{data.prev.title}</span>
						</a>
					{:else}
						<a class="navlink prev" href="{base}/{courseSlug}">
							<span class="navlink-dir">← Course home</span>
							<span class="navlink-title">{data.courseTitle}</span>
						</a>
					{/if}
					{#if data.next}
						<a class="navlink next" href="{base}/{courseSlug}/lesson/{data.next.slug}">
							<span class="navlink-dir">Next →</span>
							<span class="navlink-title">{data.next.title}</span>
						</a>
					{:else}
						<a class="navlink next finale" href="{base}/{courseSlug}">
